...
from .architectures import ABSAModel, ASPECT_COLS, PhoBERTABSABiLSTM, parse_logits
...
# Bootstrap-only default: used per-aspect when thresholds.json is missing/incomplete.
# Real per-aspect thresholds are calibrated on the validation set by
# scripts/calibrate_thresholds.py and shipped in THRESHOLDS_FILE — that file, not
# this constant, is the source of truth for production presence decisions.
_FALLBACK_PRESENCE_THRESHOLD = 0.5
THRESHOLDS_FILE = Path(__file__).resolve().parent / "thresholds.json"


def _load_calibrated_thresholds(path: Path = THRESHOLDS_FILE) -> dict[str, float]:
    if not path.exists():
        warnings.warn(
            f"No calibrated thresholds found at {path}. Falling back to "
            f"{_FALLBACK_PRESENCE_THRESHOLD} for every aspect — run "
            "packages/absa_core/scripts/calibrate_thresholds.py to generate it.",
            stacklevel=2,
        )
        return {}
...
class ABSAPredictor:
    ...
    def _is_present(self, pres_probs: torch.Tensor, aspect_idx: int) -> bool:
        ...
        aspect_col = ASPECT_COLS[aspect_idx]
        threshold = self.phobert_thresholds.get(aspect_col, _FALLBACK_PRESENCE_THRESHOLD)
        return presence_conf > threshold
    ...
    def predict(self, texts):
        ...
        if self.model_variant == "phobert":
            cleaned = [f"{ABSA_PROMPT_PREFIX}{t}" for t in cleaned]
